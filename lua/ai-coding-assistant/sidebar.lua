...
setup_diff_actions = function(parsed_diff)
  ...
  local function on_reject()
    cleanup_diff_actions()
    highlighter.clear(vim.fn.bufnr(parsed_diff.file_path, true))
...
submit_input = function()
  ...
  require("ai-coding-assistant.core").request(clean_prompt, context_block, function(response)
    -- ⭐️ THIS IS THE CORRECTED LOGIC THAT AVOIDS THE CRASH ⭐️

    -- 1. Build the final AI message as a single string.
    ...
    -- 2. Safely replace the "Thinking..." message with the final, complete message.
    -- This single assignment replaces the buggy `table.insert` calls.
