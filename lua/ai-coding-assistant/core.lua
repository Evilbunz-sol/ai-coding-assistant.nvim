...
function M.request(prompt_text, callback)
  ...
  --> This line had a typo. It should read from M.state, not M.config.
  local provider_name = M.state.current.provider
  local model_name = M.state.current.model

  --> Now we get the provider details from the correct config table.
  ...
  -- The rest of the function is correct and does not need to change.
  -- ... (The rest of the M.request function)
